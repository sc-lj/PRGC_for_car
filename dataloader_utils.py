...
class InputExample(object):
    ...
    def __init__(self, text, en_pair_list, re_list, rel2ens, pos_list=None, rel2ens2pos=None,pair_type_list=None):
        self.text = text
...
def read_examples(data_dir, data_sign, rel2idx):
    """load data to InputExamples，for CarFaultRelation data
    """
    examples = []
...
def read_examples_v2(data_dir, data_sign, rel2idx):
    """load data to InputExamples，for CMeIE data
    """
    examples = []

    # read src data
    with open(data_dir / f'CMeIE_{data_sign}.json', "r", encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            sample = json.loads(line)
            text = sample['text']
            text = jionlp.clean_text(text,remove_parentheses=False,remove_phone_number=False)
            rel2ens = defaultdict(list)
            en_pair_list = []
            pair_type_list = []
            re_list = []
...
def convert_v2(example: InputExample, max_text_len: int, tokenizer, rel2idx, data_sign, ex_params):
    """转换函数 for CMeIE data
    Args:
        example (_type_): 一个样本示例
        max_text_len (_type_): 样本的最大长度
